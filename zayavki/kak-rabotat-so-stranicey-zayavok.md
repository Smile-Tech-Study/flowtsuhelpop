---
order: 0.3
title: Как работать со страницей заявок?
---

Страница заявок для быстрого и удобного доступа к любой заявке в системе.

### Поиск

По каждому из столбцов доступен быстрый поиск. Для этого надо ввести значение и нажать Enter.

![](./kak-rabotat-so-stranicey-zayavok-6.png){width=1313px height=221px}

Например, найдем заявку с номером 99:

![](./kak-rabotat-so-stranicey-zayavok-5.png){width=1331px height=264px}

### Шаблон

Изначально содержит на странице 6 столбцов по умолчанию: номер заявки, ФИО, указание, что заявка активная, статус заявки, СНИЛС, Email.

![](./kak-rabotat-so-stranicey-zayavok.png){width=1327px height=458px}

Если необходимо добавить ещё какие-то столбы для отображения, то можно проставить галочку, а также сохранить набор этих столбцов как шаблон, чтобы они открывались автоматически. Сделать это можно следующим образом: например, добавили дополнительный столбец Дата рождения, далее нажимаем на кнопку «Сохранить».

![](./kak-rabotat-so-stranicey-zayavok-2.png){width=1317px height=440px}

Называем шаблон, сохраняем его.

![](./kak-rabotat-so-stranicey-zayavok-3.png){width=1077px height=186px}

Теперь при открытии страницы заявок будет открываться по умолчанию сохраненный набор столбцов для быстрого доступа. Сохранить можно несколько таких шаблонов для разных случаев, они будут доступны вверху страницы.

![](./kak-rabotat-so-stranicey-zayavok-4.png){width=320px height=234px}

Сбросить настройки таблицы или сбросить шаблон можно при помощи соответствующих кнопок.

![](./kak-rabotat-so-stranicey-zayavok-8.png){width=1056px height=99px}

### Экспорт

На странице заявок доступен экспорт данных в Excel. Для этого надо настроить таблицу с данными, которые Вам надо экспортировать, например, по статусу заявок. И далее нажать на кнопку.

![](./kak-rabotat-so-stranicey-zayavok-7.png){width=779px height=192px}

### Уведомления

Рассылка уведомлений через Flow. Как это сделать?

1️⃣Открыть страницу с заявками  и отфильтровать по параметрам. (У нас для примера все граждане, кому отклонили документы на зачисление)

2️⃣Кликнуть по 🔔

3️⃣ Сформировать уведомление (в нашем примере можно попросить граждан поскорее заменить отклоненные документы)

4️⃣ Выбрать канал рассылки  и "Отправить"

Также есть поле (обведено на скрине), куда мы **рекомендуем написать контакты организации**, чтобы гражданин не потерялся, если у него появятся вопросы после получения уведомления.

![](./kak-rabotat-so-stranicey-zayavok.jpeg){width=1280px height=556px}


