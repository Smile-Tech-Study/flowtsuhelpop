...
-  Первичные документы: скан главной страницы паспорта и страницы с регистрацией, документ о текущем образовании (для любого уровня образования кроме «Без образования») и, если требуется, документ о смене фамилии. ***Документы загружает сам гражданин через свой ЛК.***
...
Документы попадают в блок "Сканы документов" и проверяются сотрудниками ОП. Для удобной проверки можно воспользоваться удобной **сверкой документов**. Для этого надо нажать на значок редактирования в блоке «Данные слушателя», далее на открывшейся странице перейти во вкладку «Первичные документы». При клике на документ будет открываться блок с заполненными данными, а рядом скан этого документа, загруженный гражданином. Поочередно необходимо так проверить каждый из документов в удобном формате одного окна.

[video:https://rutube.ru/video/private/5b9e6ed89972461de30fac8617bee66c/?p=nTlEgLloP7aCzfYfAtr6Kw]

:::info 

Среди документов есть  "Договор о намерениях", он генерируется на портале РР и подписывается электронно.
...
* [x] Отфильтруйте заявки по статусу «Требуется проверить первичные документы». (Заявки - Все заявки - в столбце «Статус заявки» выбрать необходимый»).

![](./README.png){width=2746px height=422px}

* [x] Откройте страницу заявки, кликнув на номер в таблице с результатами.

* [x] Откройте каждый из документов,  загруженных в блоке "Сканы документов» через сверку документов (функционал описан чуть выше) и проверьте по каждому из документов:
...
* [x] В первичные документы есть возможность загрузки **документа, подтверждающего трудоустройство**: при выбранной форме занятости "Трудоустроен по трудовому договору" можно загрузить "Документ о трудоустройстве». После загрузки документа о трудоустройстве он будет отображаться на странице заявки в блоке "Сканы документов".

   Обращаем внимание, что "Документ о трудоустройстве" **не является обязательным** для загрузки документом.

   После загрузки документа о трудоустройстве он будет отображаться на странице заявки в блоке "Сканы документов". Сотрудник образовательной организации сможет одобрить/отклонить с указанием причины отклонения, как и остальные документы.

   **Важно**! Если отклонен ТОЛЬКО документ о трудоустройстве (при этом одобрены остальные первичные документы), заявка НЕ откатывается назад по шагам, в ЛК гражданина отобразится только причина отклонения документа. Заменить отклоненный документ гражданин сможет в разделе "Обучение". После замены его можно будет проверить снова.
...
* [x] По [инструкции](./podpisanie-dogovora-o-namereniyakh) подпишите Договор о намерениях.
